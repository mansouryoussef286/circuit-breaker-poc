...
- A lightweight decorator (`CircuitBreaker`) that can be used to mark methods for circuit-breaker behavior (metadata only in this POC).
...
Available npm scripts are defined in `package.json` (e.g. `start`, `start:prod`, `test`, `lint`).
...
- Slow / Timeout: Toggle the endpoint by calling `/toggle-todo-erroring`, then call `/external-todos` — the service simulates slow responses which will cause `opossum` timeouts and errors.
...
Quick CURL examples (when server is running on `http://localhost:3000`):

```bash
# Normal call
curl http://localhost:3000/external-todos

# Toggle to erroring slow calls
curl http://localhost:3000/toggle-todo-erroring
...
- Add method-level decorator wiring so the `@CircuitBreaker()` decorator automatically applies the circuit (not only metadata).
...
- Support for bulkhead isolation and concurrency limits for protected outbound calls.
...
- `src/circuit-breaker/circuit-breaker.decorator.ts`
